...
6. Остальные после git pull и npm install сразу видят новые тесты и могут запускать их локально командой npm test

**Закомитить код со сломанным текстом не удастся так как у нас настроен Husky pre commit**